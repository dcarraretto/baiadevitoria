...
<!-- Parte original da animação de caracteres:  Peter Gehrig  -->
<!-- Adequação para outro evento de disparada: C3s Sistemas -->
<!-- Demais funções: C3s Sistemas -->
...
function Abrir(end,w,h) {
  ...
  prop='toolbar=1,location=0,directories=0,status=0,menubar=0,scrollbars=yes,resizable=1,screenX=0,screenY=0,width=' + ww + ',height=' + hh;
	   janela=window.open(end,"win",prop)
	   }
function Is() {
...
function MM(e) {
  if (ob) {
    ...
    if (((event.clientY-Y + document.body.scrollTop) >= 0)&&((event.clientY-Y + document.body.scrollTop) + 20 <= document.body.clientHeight)){
      ...
      textoLyr.pixelTop = -(event.clientY-Y + document.body.scrollTop)*(2600/document.body.clientHeight);
      return false;
      }
    }
  }
